document.addEventListener('DOMContentLoaded', function() {
  ...
  if (hash && (hash === 'talks' || hash === 'livestreams' || hash === 'podcasts' || hash === 'webinars' || hash === 'interviews' || hash === 'tutorials')) {
    switchTab(hash);
  } else if (window.location.href.includes('#')) {
    // Handle cases where URL might be /speaking#livestreams without the slash
    const urlHash = window.location.href.split('#')[1];
    if (urlHash && (urlHash === 'talks' || urlHash === 'livestreams' || urlHash === 'podcasts' || urlHash === 'webinars' || urlHash === 'interviews' || urlHash === 'tutorials')) {
      switchTab(urlHash);
    }
  }
